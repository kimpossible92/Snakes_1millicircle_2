...
public class MySide : NetworkBehaviour {
    ...
    void Update()
    {
        if (Input.GetKeyUp(KeyCode.Space) && UnityEngine.Networking.NetworkManager.singleton.client != null)
        {
            try
            {
    ...
    void OnDestroy()
    {
        ((NetSide)UnityEngine.Networking.NetworkManager.singleton).RemoveClientHandler(MyReconn);
        ((NetSide)UnityEngine.Networking.NetworkManager.singleton).RemoveClientConnect(Connec);
        GetComponent<Prog>().serveractive = false;
    }
    public void Client()
    {
        ((NetSide)UnityEngine.Networking.NetworkManager.singleton).AddClientDisconnect(MyReconn);
        ((NetSide)UnityEngine.Networking.NetworkManager.singleton).AddClientConnect(Connec);
    }
    private void Connec()
    {
    ...
    private IEnumerator Reconn()
    {
        ...
        NetworkTransport.ConnectAsNetworkHost(UnityEngine.Networking.NetworkManager.singleton.client.connection.hostId, "", 7777, network, source, node, out error);
