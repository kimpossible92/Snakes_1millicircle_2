...
namespace MoreMountains.HighroadEngine
{
	...
	public class OnlineLobbyPlayer : NetworkLobbyPlayer, ILobbyPlayerInfo
	{
		...
		public override void OnClientEnterLobby()
		{
			...
			if (!OnlineLobbyMst.Instance._onlineLobbyUI.Lobby.gameObject.activeSelf)
			{
				// We change to connected mode
				OnlineLobbyMst.Instance._onlineLobbyUI.ShowLobby();
				OnlineLobbyMst.Instance._onlineLobbyUI.ShowConnected();
				OnlineLobbyMst.Instance._onlineLobbyUI.HidePopup();
			}

			// we look for the associated player gui zone
			RectTransform slotZone = OnlineLobbyMst.Instance._onlineLobbyUI.PlayersSelection[slot];
			...
			OnlineLobbyMst.Instance.OnLobbyServerPlayersReady();
		}
		...
		public override void OnStartClient()
		{
			...
			foreach (var prefab in OnlineLobbyMst.Instance.AvailableVehiclesPrefab)
			{
		...
		protected virtual void InitUI()
		{
			if (isLocalPlayer)
			{
				...
				if (slot == 0)
				{
					OnlineLobbyMst.Instance._onlineLobbyUI.ShowTrackSelection(this);
				}
			}
			else 
			{
		...
		public virtual void OnVehicleChange(int direction)
		{
			...
			{
				newVehicleSelected = OnlineLobbyMst.Instance.AvailableVehiclesPrefab.Length - 1;
			} 
			else if (newVehicleSelected > (OnlineLobbyMst.Instance.AvailableVehiclesPrefab.Length - 1))
			{
		...
		{
			VehicleInformation info = OnlineLobbyMst.Instance.AvailableVehiclesPrefab[_currentVehicleSelected].GetComponent<VehicleInformation>();
		...
		public virtual void OnTrackChange(int direction)
		{
			...
			if (newvalue < 0)
			{
				newvalue = OnlineLobbyMst.Instance.AvailableTracksSceneName.Length - 1;
			}
			else if (newvalue >= OnlineLobbyMst.Instance.AvailableTracksSceneName.Length) 
			{
				newvalue = 0;
			}
		...
		public virtual void OnCurrentTrackChanged(int value)
		{
			...
			OnlineLobbyMst.Instance._onlineLobbyUI.UpdateTrackInfo(_currentTrackSelected);
		}
		...
		{
			OnlineLobbyMst.Instance._onlineLobbyUI.ShowPopup("Loading " + scenename + "...");
		}
	}
}
