...
public class RacerM : MonoBehaviour
{
	...
	public virtual void Awake()
	{
		...
		{
			Debug.LogWarning("List of checkpoints should be initialized in RacerM gameobject inspector.");
		}
	...
	public virtual void Start()
	{
		...
		if (CameraChangeButton != null)
		{
			...
			CameraChangeButton.onClick.AddListener(OnCamCh);
		}
		...
		if (FindObjectOfType<OnlineLobbyMst>() != null)
		{
			_isNetworkMode = true;
			_lobbyManager = FindObjectOfType<OnlineLobbyMst>();
		...
		else
		{
			...
			_lobbyManager = FindObjectOfType<LocalLobbyMst>();

			OnDisableControlForPlayers = DisableCForPl;
			OnEnableControlForPlayers = EnableCtForPl;
			...
			if (FindObjectOfType<LocalLobbyMst>().Players().Count == 0)
			{
				InitTestMode();
			}
	...
	protected virtual void InitTestMode()
	{
		...
		if (HumanPlayer)
		{
			LocalLobbyMst.Instance.AddPlayer(new LocalLobbyPlayer
			{
	...
	protected virtual void InitPlayers()
	{
		...
		{
			playersAtStart =
					LocalLobbyMst.Instance.Players()
					.Select(x => x.Value)
		...
		{
			playersAtStart = new List<ILobbyPlayerInfo>(LocalLobbyMst.Instance.Players().Values);
		}
		...
		foreach (ILobbyPlayerInfo item in playersAtStart)
		{
			...
			{
				prefab = LocalLobbyMst.Instance.AvailableVehiclesPrefabs[item.VehicleSelectedIndex];
			}
			else
			{
		...
		{
			Debug.LogError("No camera available found. Please ensure at least one camera is configurated in RacerM inspector.");
			return;
		}
		else
		{
	...
	public virtual void EnableCtForPl()
	{
		if (_isNetworkMode)
		{
			Debug.LogError("This function should not be called in network mode");
		...
		for (int i = 0; i < _lobbyManager.MaxPlayers; i++)
		{
			...
			{
				int controllerID = -1;
				if (!LocalLobbyMst.Instance.GetPlayer(i).IsBot)
				{
					controllerID = i;
				}
	...
	public virtual void DisableCForPl()
	{
		if (_isNetworkMode)
		{
			Debug.LogError("This function should not be called in network mode");
	...
	protected virtual void OnCamCh()
	{
